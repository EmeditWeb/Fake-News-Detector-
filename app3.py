...
# --- Download and configure NLTK data ---
...
@st.cache_resource
def setup_nltk_data():
    # Define a writable path within the app's working directory for NLTK data
    ...
    # This makes sure NLTK looks here first AND downloads here by default.
    if nltk_data_path not in nltk.data.path:
        nltk.data.path.insert(0, nltk_data_path)
    
    # Inform the user (or logs) about the NLTK data path
    st.info(f"NLTK data directory set to: `{nltk_data_path}`")

    try:
        # Attempt to download stopwords and wordnet.
        # NLTK's download function is smart enough not to re-download if resources exist.
        # Removing 'quiet=True' to allow full download output in logs if issues persist.
        nltk.download('stopwords') 
        nltk.download('wordnet')
        
        # Verify that the resources are now findable.
        # This will raise a LookupError if they still can't be found after download,
        # which our except block will catch.
        nltk.data.find('corpora/stopwords')
        nltk.data.find('corpora/wordnet')
        
        st.success("NLTK data (stopwords, wordnet) ready for use! ✅")
    except Exception as e:
        # Catch any general exception during download or finding resources.
        st.error(f"Critical Error: Failed to download or access NLTK data. "
                 f"This is essential for text preprocessing. Please check your app's "
                 f"Streamlit Cloud logs for more details. Error: {e}")
        st.stop() # Stop the app execution if NLTK data is not available, as it's critical.
